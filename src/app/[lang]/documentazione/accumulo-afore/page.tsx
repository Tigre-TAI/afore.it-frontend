...
export default async function AccumuloAforePage({ params }: Props) {
  ...
  try {
    const productTypes = ['BATTERIA_DI_ACCUMULO'];
  ...
  // Filter Afore battery documents
  const aforeBatteryDocuments = (() => {
    return allDocuments.filter((doc) => {
      const category = categorizeDocument(doc.fileName);
      const fileName = doc.fileName.toLowerCase();
      // Filter for Afore batteries (not Hailei)
      return (category === 'CEI 0-21' || 
             ...
             (fileName.includes('afore') && !fileName.includes('hailei'));
    });
  })();
  ...
  const productGroups = (() => {
    ...
    aforeBatteryDocuments.forEach((doc) => {
      const fileName = doc.fileName.toLowerCase();
      let groupKey = 'afore';
      let groupTitle = 'Accumulo Afore';
  ...
  const pageTitle = translationsObj.documentazione?.accumuloAfore?.title || "Accumulo Afore";
  const pageSubtitle = translationsObj.documentazione?.accumuloAfore?.subtitle || "Documenti tecnici e certificazioni per i sistemi di accumulo Afore.";
