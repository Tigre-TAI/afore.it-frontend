...
export default async function CertificatiInverterIbridiPage({ params }: Props) {
  ...
  // Filter hybrid inverter documents
  const hybridDocuments = (() => {
  ...
  const productGroups = (() => {
    ...
    hybridDocuments.forEach((doc) => {
      ...
      if (fileName.includes('af-sl') || (fileName.includes('ibrido') && fileName.includes('monofase'))) {
        groupKey = 'afsl';
        groupTitle = 'Inverter Ibrido Monofase AF-SL';
  ...
  const pageTitle = translationsObj.documentazione?.certificatiInverterIbridi?.title || "Certificati Inverter Ibridi";
  const pageSubtitle = translationsObj.documentazione?.certificatiInverterIbridi?.subtitle || "Certificazioni e guide per l'integrazione dei sistemi ibridi proposti da Afore.";
