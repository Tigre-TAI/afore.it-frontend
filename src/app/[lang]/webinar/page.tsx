...
const WEBINAR_ITEM_KEYS = ["aforeHailei"] as const;
  ...
  return (
    <>
      <section className="relative -mt-[88px] pt-[88px]">
        <HeroBackground
          src="/image/heroes/webinar_hero.jpg"
          alt={t("webinar.title")}
        />
          ...
          <div className="flex flex-col lg:flex-row gap-8 lg:gap-12">
            <div className="flex-1 min-w-0 py-12 lg:pt-12">
              <h2 className="text-lg sm:text-xl font-semibold text-[#111827] mb-6 sm:mb-8 tracking-tight">
                {t("webinar.latestTitle")}
              </h2>
              <ul className="space-y-8">
                {WEBINAR_ITEM_KEYS.map((key) => {
                  const date = t(`webinar.items.${key}.date`);
                  const title = t(`webinar.items.${key}.title`);
                  const excerpt = t(`webinar.items.${key}.excerpt`);
                  const slug = KEY_TO_SLUG[key];
                  const linkHref = withLang(`/webinar/${slug}`, lang);
                  const image = t(`webinar.items.${key}.image`);
